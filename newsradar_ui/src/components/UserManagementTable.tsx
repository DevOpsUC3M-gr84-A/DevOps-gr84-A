...
/**
 * Componente para gestionar usuarios (Admin-only).
 * Renderiza una tabla semántica con accesibilidad WCAG.
 */
export const UserManagementTable: React.FC<UserManagementTableProps> = ({ isAdmin }) => {
    ...
    const fetchUsers = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          setError(
            errorData.detail || "Error al cargar la lista de usuarios"
          );
          setLoading(false);
          return;
        }
  ...
  const handleRoleChange = async (
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        setError(
          errorData.detail || "Error al actualizar el rol del usuario"
        );
        setUpdatingUserId(null);
        return;
      }
  ...
  return (
    <section className="user-management-section" aria-labelledby="user-management-title">
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id}>
                  ...
                  <td>{ROLE_NAMES[user.role_ids[0]] || "Desconocido"}</td>
                  <td>
                    <select
