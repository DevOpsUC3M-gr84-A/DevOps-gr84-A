...
describe('AlertForm Component', () => {
  ...
  test('muestra EDITAR ALERTA y precarga datos cuando initialData está informado', () => {
    ...
    expect(screen.getByLabelText('CATEGORIA IPTC (NIVEL 1)')).toHaveValue('Ciencia y tecnologia');
    expect(screen.getByPlaceholderText('Ej: ElPais, BBC, Reuters')).toHaveValue('Reuters');
  });
  ...
  test('no permite enviar si falta categoria IPTC', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('Debes seleccionar una categoria IPTC.');
  ...
  test('no permite enviar si la expresion cron está vacía', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('La expresion cron es obligatoria.');
    expect(mockOnSubmit).not.toHaveBeenCalled();
  });
  ...
  test('muestra error si no hay sesión disponible al guardar', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('Sesión no disponible para guardar la alerta');
    expect(mockOnSubmit).not.toHaveBeenCalled();
  });
  ...
  test('limpia los espacios, separa por comas y llama a onSubmit con el payload correcto', async () => {
    ...
    const inputFuentes = screen.getByPlaceholderText('Ej: ElPais, BBC, Reuters');

    fireEvent.change(inputNombre, { target: { value: 'Alerta Compleja' } });
    fireEvent.change(selectCategoria, { target: { value: 'Ciencia y tecnologia' } });
  ...
  test('muestra error si falla la API de recomendaciones', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('No se pudieron obtener recomendaciones');
  });
  ...
  test('muestra "No hay sugerencias nuevas" cuando recomendaciones no es array', async () => {
    ...
    expect(await screen.findByText('No hay sugerencias nuevas.')).toBeInTheDocument();
  });
  ...
  test('muestra error si falla el guardado en API', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('No se pudo guardar la alerta: Error al guardar la alerta');
    expect(mockOnSubmit).not.toHaveBeenCalled();
  });
  ...
  test('muestra error desconocido si guardar lanza un valor no Error', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('No se pudo guardar la alerta: Error desconocido');
  });
