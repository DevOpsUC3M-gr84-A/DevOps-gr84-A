...
export const VerifyEmail: React.FC = () => {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = (await response.json()) as { detail?: string | Record<string, unknown> };
          const detail =
        ...
        setStatus('success');
        setMessage('Tu cuenta ha sido verificada correctamente. Ya puedes iniciar sesion.');
      } catch (error) {
        const errorMessage =
  ...
  return (
    <main className="main-content" aria-labelledby="verify-email-title">
      <section className="table-container" style={{ maxWidth: 700, margin: '2rem auto' }}>
        ...
        {status === 'error' && (
          <div role="alert" aria-live="assertive" className="alert-feedback alert-feedback-error">
            <p>{message}</p>
            <a href="/" className="btn-secondary" style={{ display: 'inline-block', marginTop: '1rem' }}>
              Volver al Login
            </a>
          </div>
        )}
