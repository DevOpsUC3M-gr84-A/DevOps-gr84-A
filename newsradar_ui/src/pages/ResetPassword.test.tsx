...
describe('ResetPassword', () => {
  ...
  test('no envía si las contraseñas no coinciden', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Confirmar Contraseña/i), { target: { value: 'password124' } });
    ...
    await waitFor(() => {
      expect(globalThis.fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          body: JSON.stringify({ token: 'abc123', new_password: 'password123' }),
        })
      );
    });

    expect(await screen.findByRole('status')).toHaveTextContent('Tu contraseña se ha actualizado correctamente.');
  });
  ...
  test('muestra mensaje por defecto cuando falla sin detail', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('No se pudo restablecer la contraseña. Inténtalo más tarde.');
  });
  ...
  test('muestra error inesperado si se lanza un valor no Error', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('Error inesperado al restablecer la contraseña.');
  });
  ...
  test('muestra enlace para volver al login', () => {
    ...
    expect(screen.getByRole('link', { name: /Volver al Login/i })).toHaveAttribute('href', '/');
  });
  ...
  test('muestra error si el token no está presente en la URL', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('El enlace de recuperación no es válido.');
    expect(fetchSpy).not.toHaveBeenCalled();
  });
});
