...
describe('VerifyEmail Page', () => {
  ...
  test('muestra error si no hay token en la URL', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('Token de verificacion no proporcionado.');
  ...
  test('muestra estado de carga y luego éxito cuando token es válido', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(await screen.findByRole('status')).toHaveTextContent('Tu cuenta ha sido verificada correctamente.');
    });
